...
class CadastroPolicialScreen(BaseScreen):
	...
	def get_content(self) -> ft.Control:
		...
		# Campo situação deve estar sempre como 'ativo' e não aparecer na UI
		# Mantemos o controle apenas para compatibilidade interna e reloads
		...
		# Novo campo Função (dropdown)
		self.campo_funcao = ft.Dropdown(
			label="Função",
			width=200,
			options=[
		...
		form = ft.Container(
			content=ft.Column(
				controls=[
					...
					ft.Container(height=5),
					ft.Row(controls=[self.campo_escala, self.campo_funcao], alignment=ft.MainAxisAlignment.CENTER, spacing=10),
					ft.Container(height=5),
					ft.Row(controls=[self.campo_inicio, self.campo_unidade], alignment=ft.MainAxisAlignment.CENTER, spacing=10),
	...
	def on_salvar(self, e):
		nome = (self.campo_nome.value or "").strip().upper()
		qra = (self.campo_qra.value or "").strip().upper()
		...
		funcao = (self.campo_funcao.value or "").strip()
		...
		if not escala:
			...
			return
		if not funcao:
			self.show_error("Selecione a função")
			return
		if not inicio:
		...
		sucesso = self.app.db.inserir_policial(
			nome=nome,
			qra=qra,
			matricula=matricula,
			escala=escala,
	...
	def _reload_screen(self):
		...
		try:
			...
			self.campo_situacao.value = "ativo"
			self.campo_funcao.value = None
			self.campo_nome.update()
			self.campo_qra.update()
			self.campo_matricula.update()
			self.campo_escala.update()
			self.campo_situacao.update()
			self.campo_funcao.update()